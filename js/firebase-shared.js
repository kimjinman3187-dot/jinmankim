...
(function installWork22_3GFinanceCompletedPatch() {
    ...
    function renderCompletedList() {
        ...
        tbody.innerHTML = items.map(o => {
            ...
            return `<tr class='hover:bg-green-500/5 transition-colors'><td class='px-4 py-3 text-slate-400'>${paidDate}</td><td class='px-4 py-3 font-bold text-white'>${o.client || '-'}</td><td class='px-4 py-3 text-slate-400'><span class='text-white font-bold'>${window.yjGetProductName(o)}</span><br>${o.material || '-'}<br><span class='text-white font-bold'>${qty.toLocaleString()}장</span></td><td class='px-4 py-3 text-right font-black text-blue-400'>${window.yjFormatKRW(total)}</td><td class='px-4 py-3 text-right font-black text-green-400'>${window.yjFormatKRW(paid)}</td><td class='px-4 py-3 text-center'><div class='flex flex-col items-center gap-1.5'><span class='bg-green-500/10 text-green-400 border border-green-500/30 px-2 py-1 rounded text-[10px] font-black whitespace-nowrap'>입금완료</span><button onclick="window.openTradeStatement && window.openTradeStatement('${o.id}')" class='min-w-[88px] h-8 px-3 rounded-lg bg-[#0f1522] border border-[#334155] hover:border-blue-400 text-slate-300 hover:text-white text-[10px] font-black whitespace-nowrap leading-none inline-flex items-center justify-center transition-colors'>📄 거래명세서</button></div></td></tr>`;
