            ...
            <div id='pcFinanceProductionProgressSection' class='bg-[#1e293b] border border-[#334155] rounded-2xl shadow-xl overflow-hidden mb-8'>
                <div class='p-5 border-b border-[#334155] bg-[#111827] flex justify-between items-center'>
                    <div>
                        ...
                        <p class='text-[11px] text-slate-500 font-bold mt-1'>승인 완료 후 생산 진행률 확인이 필요한 주문</p>
                    </div>
    ...
    window.updatePCFinanceProductionProgressList = function updatePCFinanceProductionProgressList(metrics = {}) {
        ...
        tbody.innerHTML = items.map(o => {
            ...
            const statusBadge = pct >= 100
                ? `<span class='bg-green-500/10 text-green-400 border border-green-500/30 px-2 py-1 rounded text-[11px] font-black'>생산완료 대기</span>`
                : `<span class='bg-orange-500/10 text-orange-400 border border-orange-500/30 px-2 py-1 rounded text-[11px] font-black'>생산중</span>`;
            ...
            return `<tr class='hover:bg-orange-500/5 transition-colors cursor-pointer pc-table-row border-l-[3px] border-transparent' onclick="selectFinanceDetailRow(this, '${o.id}', event)"><td class='px-4 py-3 text-slate-400 align-top'>${esc(o.dueDate || '-')}</td><td class='px-4 py-3 font-bold text-white align-top'>${esc(o.client || '-')}</td><td class='px-4 py-3 text-slate-400 align-top min-w-[200px]'><div class='text-white font-bold'>${esc(window.yjGetProductName(o))}</div><div class='text-[11px]'>${esc(o.material || '-')}${specText ? ' · ' + esc(specText) : ''}</div>${colorMemo ? `<div class='text-[11px]'>색상/마감: ${esc(colorMemo)}</div>` : ''}${noteMemo ? `<div class='text-[11px] whitespace-pre-line'>특이사항: ${esc(noteMemo)}</div>` : ''}<div class='text-white font-bold mt-1'>${qty.toLocaleString()}장</div></td><td class='px-4 py-3 min-w-[220px] align-top'><div class='flex justify-between text-[11px] font-bold mb-1'><span class='text-slate-400'>${completedQty.toLocaleString()} / ${qty.toLocaleString()}장</span><span class='text-orange-400'>${pct}%</span></div><div class='w-full bg-[#0f1522] h-2.5 rounded-full overflow-hidden border border-[#334155]'><div class='bg-orange-500 h-full rounded-full transition-all duration-1000 ease-out' style='width: ${pct}%'></div></div><div class='mt-1 text-[11px] text-slate-500 font-bold'>잔여 ${remainQty.toLocaleString()}장</div></td><td class='px-4 py-3 text-center align-top'>${statusBadge}</td></tr>`;
...
(function installWork22_3EFinanceApprovalPatch() {
    ...
    function injectFinanceApprovalSection() {
        ...
        anchor.insertAdjacentHTML('beforebegin', `
            <div id='pcFinanceApprovalWaitSection' class='bg-[#1e293b] border border-[#334155] rounded-2xl shadow-xl overflow-hidden mb-8'>
                <div class='p-5 border-b border-[#334155] bg-[#111827] flex justify-between items-center'>
                    <div><h3 class='text-sm font-bold text-white uppercase tracking-widest'>📝 PC 신규 승인 대기 리스트</h3><p class='text-[11px] text-slate-500 font-bold mt-1'>영업 접수 후 회계 승인 또는 반려 처리가 필요한 주문</p></div>
    ...
    function renderFinanceApprovalWaitList() {
        ...
        tbody.innerHTML = items.map(o => {
            ...
            return `<tr class='hover:bg-cyan-500/5 transition-colors'><td class='px-4 py-3 text-slate-400 whitespace-nowrap align-top'>${o.dueDate || '-'}</td><td class='px-4 py-3 font-bold text-white align-top'>${o.client || '-'}</td><td class='px-4 py-3 text-slate-400 align-top min-w-[240px]'><div class='space-y-1 min-w-0'><div class='font-bold text-white text-[13px] leading-snug break-keep'>${window.yjGetProductName(o)}</div><div class='text-[11px] text-slate-400 leading-snug break-words'>${o.material || '-'}</div><div class='inline-flex items-center rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 px-2 py-0.5 text-[11px] font-black'>${qty.toLocaleString()}장</div></div></td><td class='px-4 py-3 text-right font-black text-cyan-400 align-top whitespace-nowrap'>${window.yjFormatKRW(amount)}</td><td class='px-4 py-3 text-center align-top'><span class='bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 px-2 py-1 rounded text-[11px] font-black whitespace-nowrap'>승인대기</span></td><td class='px-4 py-3 text-center align-top'><div class='flex justify-center items-center gap-2 whitespace-nowrap'>${rejectButton}${approveButton}</div></td></tr>`;
...
(function installWork22_3FFinanceCollectionPatch() {
    ...
    function renderCollectionWaitList() {
        ...
        tbody.innerHTML = items.map(item => {
            ...
            return `<tr class='hover:bg-amber-500/5 transition-colors'><td class='px-4 py-3 text-slate-400'>${o.payDate || o.dueDate || '-'}</td><td class='px-4 py-3 font-bold text-white'>${o.client || '-'}</td><td class='px-4 py-3 text-slate-400'><span class='text-white font-bold'>${window.yjGetProductName(o)}</span><br>${o.material || '-'}<br><span class='text-white font-bold'>${qty.toLocaleString()}장</span></td><td class='px-4 py-3 text-right font-black text-blue-400'>${window.yjFormatKRW(item.total)}</td><td class='px-4 py-3 min-w-[190px]'><div class='flex justify-between text-[11px] font-bold mb-1'><span class='text-slate-400'>${window.yjFormatKRW(item.paid)}</span><span class='text-amber-400'>잔금 ${window.yjFormatKRW(item.remaining)}</span></div><div class='w-full bg-[#0f1522] h-2.5 rounded-full overflow-hidden border border-[#334155]'><div class='bg-amber-500 h-full rounded-full transition-all duration-1000 ease-out' style='width: ${pct}%'></div></div></td><td class='px-4 py-3 text-center'>${paymentButton}</td></tr>`;
...
(function installWork22_3GFinanceCompletedPatch() {
    ...
    function injectCompletedSection() {
        ...
        anchor.insertAdjacentHTML('afterend', `
            <div id='pcFinanceCompletedSection' class='bg-[#1e293b] border border-[#334155] rounded-2xl shadow-xl overflow-hidden mt-8 mb-8'>
                <div class='p-5 border-b border-[#334155] bg-[#111827] flex justify-between items-center'><div><h3 class='text-sm font-bold text-white uppercase tracking-widest'>✅ PC 완료 거래 리스트</h3><p class='text-[11px] text-slate-500 font-bold mt-1'>입금 완료되어 거래 종료 상태로 분리된 주문</p></div><span id='pcFinanceCompletedMeta' class='text-[11px] font-black text-green-400 bg-green-500/10 border border-green-500/20 px-3 py-1.5 rounded-lg'>0건</span></div>
    ...
    function renderCompletedList() {
        ...
        tbody.innerHTML = items.map(o => {
            ...
            return `<tr class='hover:bg-green-500/5 transition-colors'><td class='px-4 py-3 text-slate-400'>${paidDate}</td><td class='px-4 py-3 font-bold text-white'>${o.client || '-'}</td><td class='px-4 py-3 text-slate-400'><span class='text-white font-bold'>${window.yjGetProductName(o)}</span><br>${o.material || '-'}<br><span class='text-white font-bold'>${qty.toLocaleString()}장</span></td><td class='px-4 py-3 text-right font-black text-blue-400'>${window.yjFormatKRW(total)}</td><td class='px-4 py-3 text-right font-black text-green-400'>${window.yjFormatKRW(paid)}</td><td class='px-4 py-3 text-center'><span class='bg-green-500/10 text-green-400 border border-green-500/30 px-2 py-1 rounded text-[11px] font-black whitespace-nowrap'>입금완료</span></td></tr>`;
