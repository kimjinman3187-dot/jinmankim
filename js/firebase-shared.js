...
function initializeFirebase() {
    ...
    window.db = db;
    window.auth = auth;
    window.functions = typeof firebase.functions === 'function'
...
(function installWork22_3EFinanceApprovalPatch() {
    ...
    function renderFinanceApprovalWaitList() {
        ...
        if (!window.__yjRegApproval) { (window.__yjFinanceRefreshers = window.__yjFinanceRefreshers || []).push(renderFinanceApprovalWaitList); window.__yjRegApproval = true; } // WORK52-1
        ...
        if (!tbody) return;
        const base = typeof window.yjExcludeTest === 'function' ? window.yjExcludeTest(pendingOrdersCache) : pendingOrdersCache; // WORK52-1 테스트 제외
        const items = [...base].sort((a, b) => String(a.dueDate || '').localeCompare(String(b.dueDate || '')) || Number(b.createdAt || 0) - Number(a.createdAt || 0));
        if (meta) meta.textContent = `${items.length}건`;
        tbody.innerHTML = items.map(o => {
            const qty = Number(o.qty) || 0;
            ...
            return `<tr class='hover:bg-cyan-500/5 transition-colors'><td class='px-4 py-3 text-slate-400 whitespace-nowrap align-top'>${o.dueDate || '-'}</td><td class='px-4 py-3 font-bold text-white align-top'>${o.client || '-'}${typeof window.yjTestBadge === 'function' ? window.yjTestBadge(o) : ''}</td><td class='px-4 py-3 text-slate-400 align-top min-w-[240px]'><div class='space-y-1 min-w-0'><div class='font-bold text-white text-[13px] leading-snug break-keep'>${window.yjGetProductName(o)}</div><div class='text-[11px] text-slate-400 leading-snug break-words'>${o.material || '-'}</div><div class='inline-flex items-center rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 px-2 py-0.5 text-[10px] font-black'>${qty.toLocaleString()}장</div></div></td><td class='px-4 py-3 text-right font-black text-cyan-400 align-top whitespace-nowrap'>${window.yjFormatKRW(amount)}</td><td class='px-4 py-3 text-center align-top'><span class='bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 px-2 py-1 rounded text-[10px] font-black whitespace-nowrap'>승인대기</span></td><td class='px-4 py-3 text-center align-top'><div class='flex justify-center items-center gap-2 whitespace-nowrap'>${rejectButton}${approveButton}</div></td></tr>`;
...
(function installWork22_3FFinanceCollectionPatch() {
    ...
    function renderCollectionWaitList() {
        ...
        window.yjPatchFooterVersion();
        if (!window.__yjRegCollection) { (window.__yjFinanceRefreshers = window.__yjFinanceRefreshers || []).push(renderCollectionWaitList); window.__yjRegCollection = true; } // WORK52-1
        ...
        if (!tbody) return;
        const collectionBase = typeof window.yjExcludeTest === 'function' ? window.yjExcludeTest(collectionOrdersCache) : collectionOrdersCache; // WORK52-1 테스트 제외
        const items = collectionBase.filter(o => window.yjIsInvoiceIssued(o) && o.paymentStatus !== 'paid').map(o => {
        ...
        tbody.innerHTML = items.map(item => {
            ...
            const paymentButton = (typeof window.canManagePayment === 'function' && window.canManagePayment())
                ? `<button data-yj-action='payment' data-yj-id='${o.id}' class='min-w-[88px] h-10 px-4 rounded-xl bg-amber-500 hover:bg-amber-400 text-white text-[12px] font-black whitespace-nowrap leading-none inline-flex items-center justify-center shadow-md transition-colors'>입금 등록</button>`
                : `<button onclick="if(typeof selectRow==='function')selectRow(this.closest('tr'), '${o.id}')" class='min-w-[88px] h-10 px-4 rounded-xl bg-[#0f1522] border border-[#334155] text-slate-300 hover:text-white text-[12px] font-black whitespace-nowrap leading-none inline-flex items-center justify-center'>이력 보기</button>`; // WORK52-2: 영업은 조회만
            return `<tr class='hover:bg-amber-500/5 transition-colors'><td class='px-4 py-3 text-slate-400'>${o.payDate || o.dueDate || '-'}</td><td class='px-4 py-3 font-bold text-white'>${o.client || '-'}${typeof window.yjTestBadge === 'function' ? window.yjTestBadge(o) : ''}</td><td class='px-4 py-3 text-slate-400'><span class='text-white font-bold'>${window.yjGetProductName(o)}</span><br>${o.material || '-'}<br><span class='text-white font-bold'>${qty.toLocaleString()}장</span></td><td class='px-4 py-3 text-right font-black text-blue-400'>${window.yjFormatKRW(item.total)}</td><td class='px-4 py-3 min-w-[190px]'><div class='flex justify-between text-[10px] font-bold mb-1'><span class='text-slate-400'>${window.yjFormatKRW(item.paid)}</span><span class='text-amber-400'>잔금 ${window.yjFormatKRW(item.remaining)}</span></div><div class='w-full bg-[#0f1522] h-2.5 rounded-full overflow-hidden border border-[#334155]'><div class='bg-amber-500 h-full rounded-full transition-all duration-1000 ease-out' style='width: ${pct}%'></div></div></td><td class='px-4 py-3 text-center'><div class='flex flex-col items-center gap-1.5'>${paymentButton}<button onclick="window.openTradeStatement && window.openTradeStatement('${o.id}')" class='min-w-[88px] h-9 px-4 rounded-xl bg-[#0f1522] border border-[#334155] hover:border-blue-400 text-slate-200 hover:text-white text-[11px] font-black whitespace-nowrap leading-none inline-flex items-center justify-center transition-colors'>📄 거래명세서</button></div></td></tr>`;
...
(function installWork22_3GFinanceCompletedPatch() {
    ...
    function renderCompletedList() {
        ...
        window.yjPatchFooterVersion();
        if (!window.__yjRegCompleted) { (window.__yjFinanceRefreshers = window.__yjFinanceRefreshers || []).push(renderCompletedList); window.__yjRegCompleted = true; } // WORK52-1
        const tbody = document.getElementById('pcFinanceCompletedBody');
        const meta = document.getElementById('pcFinanceCompletedMeta');
        if (!tbody) return;
        const completedBase = typeof window.yjExcludeTest === 'function' ? window.yjExcludeTest(completedOrdersCache) : completedOrdersCache; // WORK52-1 테스트 제외
        const items = [...completedBase].filter(o => o.paymentStatus === 'paid').sort((a, b) => {
        ...
        tbody.innerHTML = items.map(o => {
            ...
            return `<tr class='hover:bg-green-500/5 transition-colors cursor-pointer' onclick="if(typeof selectRow==='function')selectRow(this, '${o.id}')"><td class='px-4 py-3 text-slate-400'>${paidDate}</td><td class='px-4 py-3 font-bold text-white'>${o.client || '-'}${typeof window.yjTestBadge === 'function' ? window.yjTestBadge(o) : ''}</td><td class='px-4 py-3 text-slate-400'><span class='text-white font-bold'>${window.yjGetProductName(o)}</span><br>${o.material || '-'}<br><span class='text-white font-bold'>${qty.toLocaleString()}장</span></td><td class='px-4 py-3 text-right font-black text-blue-400'>${window.yjFormatKRW(total)}</td><td class='px-4 py-3 text-right font-black text-green-400'>${window.yjFormatKRW(paid)}</td><td class='px-4 py-3 text-center'><div class='flex flex-col items-center gap-1.5'><span class='bg-green-500/10 text-green-400 border border-green-500/30 px-2 py-1 rounded text-[10px] font-black whitespace-nowrap'>입금완료</span><button onclick="window.openTradeStatement && window.openTradeStatement('${o.id}')" class='min-w-[88px] h-8 px-3 rounded-lg bg-[#0f1522] border border-[#334155] hover:border-blue-400 text-slate-300 hover:text-white text-[10px] font-black whitespace-nowrap leading-none inline-flex items-center justify-center transition-colors'>📄 거래명세서</button></div></td></tr>`;
